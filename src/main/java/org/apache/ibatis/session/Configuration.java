...
public class Configuration {
  ...
  /**
   * 映射后sql语句的注册表 key为(namespace.sql语句的id), value为对应的MappedStatement
   */
  ...
  /*
   * A map holds cache-ref relationship. The key is the namespace that references a cache bound to another
   * namespace and
  ...
  public Configuration() {
    // 注册类型别名 什么样的类型会在这里注册,
    // 2024年06月01日23:18:06,在xml配置中可以使用别名关联相关的类
    ...
    public V put(String key,
      ...
      if (containsKey(key)) {
        throw new IllegalArgumentException(name + " already contains key " + key
          + (conflictMessageProducer == null ? "" : conflictMessageProducer.apply(super.get(key),
          value)));
      }
      if (key.contains(".")) {
